...
const Frdialogmodal = function ({
  ...
  function _addA11y(container) {
    const modal = _q(modalSelector, container)[0];
  ...
  function _handleTabEvent(e) {
    ...
    const focusedIndex = focusableElements.indexOf(doc.activeElement);
  ...
  function _eventOpenPointer(e) {
    ...
    const container = doc.getElementById(button.getAttribute('aria-controls'));
    const modal = _q(modalSelector, container)[0];
  ...
  function _eventContainerPointer(e) {
    const container = currModal.parentElement;
  ...
  function _unbindOpenPointers(container) {
    const id = container.getAttribute('id');
    const buttons = doc.querySelectorAll(`${openSelector}[aria-controls="${id}"]`);
